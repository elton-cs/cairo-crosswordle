...
impl cainome::cairo_serde::CairoSerde for LetterStatus {
    type RustType = Self;
    const SERIALIZED_SIZE: std::option::Option<usize> = None;
    #[inline]
    fn cairo_serialized_size(__rust: &Self::RustType) -> usize {
        ...
        __size += Status::cairo_serialized_size(&__rust.status);
        __size
    }
    fn cairo_serialize(__rust: &Self::RustType) -> Vec<starknet::core::types::Felt> {
        let mut __out: Vec<starknet::core::types::Felt> = vec![];
        __out.extend(u8::cairo_serialize(&__rust.position));
        __out.extend(Status::cairo_serialize(&__rust.status));
        __out
    }
    fn cairo_deserialize(
        __felts: &[starknet::core::types::Felt],
        ...
        let status = Status::cairo_deserialize(__felts, __offset)?;
        __offset += Status::cairo_serialized_size(&status);
        Ok(LetterStatus { position, status })
    }
}
#[derive(Debug, Component)]
pub enum Layout {
...
impl cainome::cairo_serde::CairoSerde for Status {
    type RustType = Self;
    const SERIALIZED_SIZE: std::option::Option<usize> = std::option::Option::None;
    #[inline]
    fn cairo_serialized_size(__rust: &Self::RustType) -> usize {
    ...
    fn cairo_serialize(__rust: &Self::RustType) -> Vec<starknet::core::types::Felt> {
        match __rust {
            Status::Hidden => usize::cairo_serialize(&0usize),
            Status::Solved => usize::cairo_serialize(&1usize),
            _ => vec![],
        }
    }
    fn cairo_deserialize(
        ...
        let __index = u128::from_be_bytes(__f.to_bytes_be()[16..].try_into().unwrap());
        match __index as usize {
            0usize => Ok(Status::Hidden),
            1usize => Ok(Status::Solved),
            _ => {
                return Err(cainome::cairo_serde::Error::Deserialize(format!(
                    "Index not handle for enum {}",
                    "Status"
